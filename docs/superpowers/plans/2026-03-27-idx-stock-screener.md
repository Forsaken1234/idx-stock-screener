...
**Tech Stack:** Python 3.11+, FastAPI, APScheduler, yfinance, pandas-ta, SQLite, React 18, Vite, TypeScript, TailwindCSS, lightweight-charts, React Query v5
...
│       │   ├── stocks.ts        # useStocks, useStock hooks
│       │   ├── market.ts        # useMarket hook
│       │   └── watchlist.ts     # useWatchlist, useAddToWatchlist, useRemoveFromWatchlist
│       ├── components/
│       │   ├── Layout.tsx       # outer wrapper: sidebar + main content
│       │   ├── Sidebar.tsx      # nav links, market status badge, last updated
│       │   ├── StaleWarning.tsx # yellow banner when data > 10 min old
│       │   ├── MetricCard.tsx   # KPI display card
│       │   ├── StockTable.tsx   # sortable/filterable table
│       │   └── Chart.tsx        # lightweight-charts wrapper (line + candlestick modes)
│       ├── pages/
│       │   ├── Dashboard.tsx
│       │   ├── Screener.tsx
│       │   ├── Watchlist.tsx
│       │   └── StockDetail.tsx
│       └── utils/
│           └── time.ts          # WIB conversion, isMarketOpen()
└── .gitignore
```
...
Update `frontend/tsconfig.json` to include vitest globals — add `"types": ["vitest/globals"]` inside `compilerOptions`:
```json
{
  "compilerOptions": {
...
```
(Merge with the existing `tsconfig.json` generated by `react-ts` template — do not replace it wholesale, just add `"vitest/globals"` to the `"types"` array in `compilerOptions`.)
...
export interface Filters {
  index?: string
  pe_max?: string | number
  rsi_min?: string | number
  rsi_max?: string | number
  [key: string]: string | number | undefined
}

export function useStocks(filters: Filters = {}) {
...
export function useWatchlist() {
  return useQuery<string[]>({
    queryKey: ['watchlist'],
    queryFn: () => fetch('/api/watchlist').then(r => r.json()).then((d: { tickers: string[] }) => d.tickers),
  })
}
...
- [ ] **Step 4: Write tests for time.ts**

Create `frontend/src/__tests__/time.test.ts`:
```ts
...
- Create: `frontend/src/components/StaleWarning.tsx`
...
- [ ] **Step 3: Create StaleWarning.tsx**
...
- [ ] **Step 5: Create Chart.tsx (lightweight-charts wrapper)**
...
const COLUMNS = [
  ...
  { key: 'price', label: 'Price', fmt: (v: any) => v?.toLocaleString('id-ID') },
  { key: 'change_pct', label: 'Chg%', fmt: (v: any) => v != null ? `${v > 0 ? '+' : ''}${v.toFixed(2)}%` : '—', color: (v: any) => v > 0 ? 'text-emerald-400' : v < 0 ? 'text-red-400' : '' },
  { key: 'pe', label: 'PE', fmt: (v: any) => v?.toFixed(1) },
  { key: 'pbv', label: 'PBV', fmt: (v: any) => v?.toFixed(2) },
  { key: 'roe', label: 'ROE%', fmt: (v: any) => v != null ? `${(v * 100).toFixed(1)}%` : '—' },
  { key: 'rsi', label: 'RSI', fmt: (v: any) => v?.toFixed(1), color: (v: any) => v < 30 ? 'text-emerald-400 font-bold' : v > 70 ? 'text-red-400 font-bold' : '' },
  { key: 'market_cap', label: 'Mkt Cap', fmt: (v: any) => v != null ? `${(v / 1e12).toFixed(1)}T` : '—' },
]
